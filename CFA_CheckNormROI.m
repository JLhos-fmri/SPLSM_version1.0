function CFA_CheckNormROI

ansct = questdlg('CT mode','CT mode','yes','no','no');
ansWithT1 = questdlg('With T1 mode','With T1 mode','yes','no','no');
PG = uigetdir(pwd,'Check Data(after normalize)');
dir1 = [PG,filesep,'NormBG',filesep];
if ~isfolder(dir1)
    dir1temp = uigetdir(pwd,'Please Select Normalized Background Image directory');
    dir1 = [dir1temp,filesep];
end
dir2 = [PG,filesep,'NormROI',filesep];
if ~isfolder(dir2)
    dir2temp = uigetdir(pwd,'Please Select Normalized ROI Image directory');
    dir2 = [dir2temp,filesep];
end

dir3 = [PG,filesep,'NormBGT1',filesep];
if ~isfolder(dir3)&&strcmp(ansWithT1,'yes')
    dir3temp = uigetdir(pwd,'Please Select Normalized Background Image directory');
    dir3 = [dir3temp,filesep];
end
file1 = dir([dir1,'*.nii']);
file2 = dir([dir2,'*.nii']);
file3 = dir([dir3,'*.nii']);
...
disp('Check Normalized ROI finished!')
end