function CFA_MakeRes_Clustercorrected_vlsm(C05,C05P,C05N,PmapDir,TmapDir,ZmapDir,THRESHOLD,VQ,Voxdir2out05)

[VP DP] = Dynamic_read_dir_NIFTI(PmapDir);
[VT DT] = Dynamic_read_dir_NIFTI(TmapDir);
[VZ DZ] = Dynamic_read_dir_NIFTI(ZmapDir);
[pth nam ext] = fileparts(TmapDir);
[pth znam ext] = fileparts(ZmapDir);

THR1P = find(DP<THRESHOLD&DT>0);
THR1N = find(DP<THRESHOLD&DT<0);
THR1 = zeros(size(DP));
THR1_P = zeros(size(DP));
THR1_N = zeros(size(DP));
THR1(THR1P) = DT(THR1P);
THR1(THR1N) = DT(THR1N);
THR1_P(THR1P) = DT(THR1P);
THR1_N(THR1N) = DT(THR1N);

ZTHR1 = zeros(size(DP));
ZTHR1_P = zeros(size(DP));
ZTHR1_N = zeros(size(DP));
ZTHR1(THR1P) = DZ(THR1P);
ZTHR1(THR1N) = DZ(THR1N);
ZTHR1_P(THR1P) = DZ(THR1P);
ZTHR1_N(THR1N) = DZ(THR1N);

THR1_Reshape = reshape(THR1,VQ.dim);
THR1_P_Reshape = reshape(THR1_P,VQ.dim);
THR1_N_Reshape = reshape(THR1_N,VQ.dim);

ZTHR1_Reshape = reshape(ZTHR1,VQ.dim);
ZTHR1_P_Reshape = reshape(ZTHR1_P,VQ.dim);
ZTHR1_N_Reshape = reshape(ZTHR1_N,VQ.dim);
...
if P001num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P001num)
        OutMap(THR1_C001{i}) = THR1_Reshape(THR1_C001{i}); 
        ZOutMap(THR1_C001{i}) = ZTHR1_Reshape(THR1_C001{i});
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Both_C0001.nii'])
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Both_C0001.nii'])
end
if P005num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P005num)
        ...
        ZOutMap(THR1_C005{i}) = ZTHR1_Reshape(THR1_C005{i});     
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Both_C0005.nii'])  
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Both_C0005.nii'])  
end
if P01num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P01num)
        ...
        ZOutMap(THR1_C01{i}) = ZTHR1_Reshape(THR1_C01{i});   
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Both_C001.nii'])    
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Both_C001.nii'])
end
if P05num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P05num)
        ...
        ZOutMap(THR1_C05{i}) = ZTHR1_Reshape(THR1_C05{i});    
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Both_C005.nii'])   
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Both_C005.nii']) 
end
...
if P001num>0
    ...
    for i = 1:length(P001num)
        ...
        ZOutMap(THR1_C001P{i}) = ZTHR1_Reshape(THR1_C001P{i});    
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Pos_C0001.nii'])
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Pos_C0001.nii'])
end
if P005num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P005num)
        ...
        ZOutMap(THR1_C005P{i}) = ZTHR1_Reshape(THR1_C005P{i});     
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Pos_C0005.nii'])   
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Pos_C0005.nii']) 
end
if P01num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P01num)
        ...
        ZOutMap(THR1_C01P{i}) = ZTHR1_Reshape(THR1_C01P{i});   
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Pos_C001.nii']) 
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Pos_C001.nii'])   
end
if P05num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P05num)
        ...
        ZOutMap(THR1_C05P{i}) = ZTHR1_Reshape(THR1_C05P{i});    
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Pos_C005.nii'])   
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Pos_C005.nii']) 
end
...
if P001num>0
    ...
    for i = 1:length(P001num)
        ...
        ZOutMap(THR1_C001N{i}) = ZTHR1_Reshape(THR1_C001N{i});         
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Neg_C0001.nii'])
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Neg_C0001.nii'])
end
if P005num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P005num)
        ...
        ZOutMap(THR1_C005N{i}) = ZTHR1_Reshape(THR1_C005N{i});       
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Neg_C0005.nii'])  
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Neg_C0005.nii'])    
end
if P01num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P01num)
        ...
        ZOutMap(THR1_C01N{i}) = ZTHR1_Reshape(THR1_C01N{i});       
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Neg_C001.nii'])    
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Neg_C001.nii'])  
end
if P05num>0
    OutMap = zeros(VQ.dim);
    ZOutMap = zeros(VQ.dim);
    for i = 1:length(P05num)
        ...
        ZOutMap(THR1_C05N{i}) = ZTHR1_Reshape(THR1_C05N{i});     
    end
    DynamicBC_write_NIFTI(OutMap,VQ,[Voxdir2out05,nam,'marked_Neg_C005.nii'])  
    DynamicBC_write_NIFTI(ZOutMap,VQ,[Voxdir2out05,znam,'marked_Neg_C005.nii'])    
end
