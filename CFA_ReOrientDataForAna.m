...
for i = 1:length(sfold)-2
    ...
    if modality~=4
        ...
        elseif round(vRsize*10000)~=round(vIsize*10000)
            ...
            [V1 D1] = Dynamic_read_dir_NIFTI([PGout,filesep,sfold(i+2).name,filesep,'ROI.nii']);
            [V2 D2] = Dynamic_read_dir_NIFTI([PGout,filesep,sfold(i+2).name,filesep,'image.nii']);
            if any(sum(V1.mat-V2.mat))
                DynamicBC_write_NIFTI(reshape(D1,V2.dim),V2,[PGout,filesep,sfold(i+2).name,filesep,'ROI.nii']);
            end
        ...
        elseif round(vRsize*10000)~=round(vOsize*10000)
            ...
            [V1 D1] = Dynamic_read_dir_NIFTI([PGout,filesep,sfold(i+2).name,filesep,'ROI.nii']);
            [V2 D2] = Dynamic_read_dir_NIFTI([PGout,filesep,sfold(i+2).name,filesep,'OtherMode.nii']);
            if any(sum(V1.mat-V2.mat))
                DynamicBC_write_NIFTI(reshape(D1,V2.dim),V2,[PGout,filesep,sfold(i+2).name,filesep,'ROI.nii']);
            end
        end
...
if errnum1>1
    for i = 1:length(errlist1)
        disp(['different dimensions for ROI and image: ',errlist1{i}]);
    end
end
if errnum2>1
    for i = 1:length(errlist2)
        disp(['different dimensions for ROI and image: ',errlist2{i}]);
    end
end
disp('Re-Orient Finished!')
end